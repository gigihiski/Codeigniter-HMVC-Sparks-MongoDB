 ...
 * @modified	Gigih Iski Prasetyawan
 * @version		Version 0.3
 */
...
class Mongo_db
{
	 ...
	 */
	 ...
	 public function where_not($field = "", $not = '')
	 {
	 	$this->where_init($field);
	 	$this->wheres[$field]['$not'] = $not;
	 	return($this);
	 }
	/**
	 *	--------------------------------------------------------------------------------
	...
	*/
	 ...
	 */
	 ...
	 public function getOne($collection = "")
	 {
	 	...
	 	$results = array();
	 	$documents = $this->db->{$collection}->findOne($this->wheres, $this->selects);
	 ...
	 */
	 ...
	 public function update_exists($collection = "", $data = array())
	 {
	 	if(empty($collection))
	 	{
	 		show_error("No Mongo collection selected to update", 500);
	 	...
	 	try
	 	{
	 		$this->db->{$collection}->update($this->wheres, array('$exists' => $true), array('$set' => $data), array('fsync' => TRUE, 'multiple' => FALSE));
	 		return(TRUE);
	 	} 
	 	catch(MongoCursorException $e)
	 	{
	 ...
	 *	--------------------------------------------------------------------------------
	 *
	 *	Push a document into the passed collection
	 *
	 *	@usage = $this->mongo_db->push('foo', $data = array());
	 */
	
	 public function push($collection = "", $data = array())
	 {
	 	if(empty($collection))
	 	{
	 		show_error("No Mongo collection selected to push", 500);
	 	}
	 	
	 	if(count($data) == 0 || !is_array($data))
	 	{
	 		show_error("Nothing to update in Mongo collection or push is not an array", 500);
	  }
	 	
	 	try
	 	{
	 		$this->db->{$collection}->update($this->wheres, array('$push' => $data));
	 		return(TRUE);
	 	} 
	 	catch(MongoCursorException $e)
	 	{
	 ...
	 *	--------------------------------------------------------------------------------
	 *
	 *	Pull a document into the passed collection
	 *
	 *	@usage = $this->mongo_db->pull('foo', $data = array());
	 */
	
	 public function pull($collection = "", $data = array())
	 {
	 	if(empty($collection))
	 	{
	 		show_error("No Mongo collection selected to pull", 500);
	 	}
	 	
	 	if(count($data) == 0 || !is_array($data))
	 	{
	 		show_error("Nothing to update in Mongo collection or pull is not an array", 500);
	  }
	 	
	 	try
	 	{
	 		$this->db->{$collection}->update($this->wheres, array('$pull' => $data));
	 		return(TRUE);
	 	} 
	 	catch(MongoCursorException $e)
	 	{
